...
public class ItemKTek extends ItemCooldown implements ICustomHoldPose {
    ...
    public ActionResult<ItemStack> func_77659_a(World worldIn, EntityPlayer playerIn, EnumHand handIn) {
        if (!showDurabilityBar(playerIn.func_184586_b(handIn))) {
            if (!worldIn.field_72995_K) {
                for (int i = 0; i < 8; i++) {
                    EntityTekBullet shot = new EntityTekBullet(worldIn, playerIn);
                    shot.setThrower(playerIn);
                    shot.func_184538_a(playerIn, playerIn.field_70125_A, playerIn.field_70177_z, 0.0f, 2.0f, 10.0f);
    ...
    public void func_77624_a(ItemStack stack, @Nullable World worldIn, List<String> tooltip, ITooltipFlag flagIn) {
        ...
        tooltip.add(TextFmt.Italic + "Tek Bullets remove a random active potion on targets.");
    }
}
