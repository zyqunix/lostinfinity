...
/* JADX INFO: loaded from: lostinfinity-1.16.4.jar:xol/lostinfinity/gui/containers/ContainerSapEvaporator.class */
public class ContainerSapEvaporator extends Container {
    private final TileEntitySapEvaporator tileentity;
    private int cookTime;
    private int totalCookTime;
    private int burnTime;
    private int currentBurnTime;

    public ContainerSapEvaporator(InventoryPlayer player, TileEntitySapEvaporator tileentity) {
        this.tileentity = tileentity;
        func_75146_a(new Slot(tileentity, 0, 17, 21));
        func_75146_a(new SlotSapEvaporatorFuel(tileentity, 1, 17, 57));
        func_75146_a(new SlotSapEvaporatorOutput(player.field_70458_d, tileentity, 2, 98, 39));
        for (int y = 0; y < 3; y++) {
            for (int x = 0; x < 9; x++) {
    ...
    public ItemStack func_82846_b(EntityPlayer playerIn, int index) {
        ...
        if (slot != null && slot.func_75216_d()) {
            ...
            if (index == 2) {
                ...
                if (SapEvaporatorRecipes.getResult(itemstack1) != null && !SapEvaporatorRecipes.getResult(itemstack1).func_190926_b()) {
                    if (!func_75135_a(itemstack1, 0, 1, false)) {
                        return ItemStack.field_190927_a;
                    }
                } else if (TileEntitySapEvaporator.isItemFuel(itemstack1)) {
