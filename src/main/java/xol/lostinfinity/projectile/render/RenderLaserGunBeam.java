...
public class RenderLaserGunBeam<T extends Entity> extends Render<T> {
    ...
    public void func_76986_a(T entity, double x, double y, double z, float entityYaw, float partialTicks) {
        ...
        if (attackEntity.counter > 22) {
            segmentHeight = ((0.004f + (0.003f * (attackEntity.counter / 2.0f))) * (attackEntity.counter - 22)) + 0.08f;
        }
        ...
        if (attackEntity.counter > 2) {
            ...
            if (attackEntity.counter > 32) {
                alpha = 1.0f - (0.125f * (attackEntity.counter - 32));
            }
            ...
            for (int i = 0; i < ((int) Math.floor(dist / 1.0f)); i++) {
                ...
                double yPos = ((y + ((dir.field_72448_b * ((double) i)) * ((double) 1.0f))) - ((double) (segmentHeight / 2.0f))) + 0.5d;
